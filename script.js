...
function setupProjectFilters() {
    ...
    buttons.forEach(function (button) {
        button.addEventListener("click", function () {
            ...
            showFilteredProjects(selectedFilter, buttons, projects, emptyMessage);
        });
    });
}

function showFilteredProjects(selectedFilter, buttons, projects, emptyMessage) {
    buttons.forEach(function (button) {
        const isActive = button.getAttribute("data-filter") === selectedFilter;
...
function setupServicePrefill() {
    const serviceLinks = document.querySelectorAll("[data-project-type]");
    const projectTypeSelect = document.getElementById("projectType");

    serviceLinks.forEach(function (link) {
        link.addEventListener("click", function () {
            const projectType = link.getAttribute("data-project-type");
...
function setupWhatsAppLinks() {
    updateWhatsAppLinks(getDefaultWhatsAppMessage());
}
...
function getDefaultWhatsAppMessage() {
    return "Hi Hakim, I'd like to discuss a website project.\n\nI'd like to know more about working with you.";
}

function buildWhatsAppMessage(formValues) {
    let message = "Hi Hakim, I'd like to discuss a website project.\n\n";
    message += "Project type: " + formValues.projectType + "\n";
    message += "Business: " + formValues.business + "\n";
    message += "Name: " + formValues.name + "\n";
    message += "Email: " + formValues.email + "\n";

    if (formValues.website) {
        message += "Website: " + formValues.website + "\n";
    }
...
function setupContactForm() {
    ...
    form.addEventListener("submit", function (event) {
        ...
        const whatsappMessage = buildWhatsAppMessage(formValues);
        updateWhatsAppLinks(whatsappMessage);

        if (successEmail) {
            successEmail.href = buildMailtoUrl(formValues);
        }
...
function getFormValues(form) {
    ...
    return {
        ...
        website: normaliseWebsite((formData.get("website") || "").toString().trim()),
...
function validateContactForm(formValues) {
    ...
    if (formValues.business.length < 2) {
        errors.business = "Please enter your business or company name.";
    }

    if (formValues.website && !isValidWebsite(formValues.website)) {
    ...
    if (formValues.message.length < 10) {
        errors.message = "Please add a short description of what you need.";
    }
...
function clearFormErrors(form) {
    const wrappers = form.querySelectorAll(".form-field");
...
function buildMailtoUrl(formValues) {
    const subject = "Website project: " + formValues.projectType;
    const body = buildWhatsAppMessage(formValues);
